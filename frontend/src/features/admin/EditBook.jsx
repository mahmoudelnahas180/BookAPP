...
import { getBookById, updateBook } from "../../services/bookServies";
      ...
      try {
        ...
        if (bookData && bookData.book) {
          ...
          reset({
            ...
            price: book.price,
            stock: book.stock,
            category: book.category?._id || book.category, // Handle populated vs unpopulated
            isFeatured: book.isFeatured,
            isSale: book.isSale,
            discountParcent: book.discountParcent,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const payload = {
        ...
        isSale: data.isSale,
        discountParcent: data.isSale ? String(data.discountParcent) : "0",
      ...
      const errorMessage =
        error.response?.data?.message || "فشل تحديث الكتاب. حاول مرة أخرى.";
      alert(errorMessage);
    }
  };
