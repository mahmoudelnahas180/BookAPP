  ...
  useEffect(() => {
    ...
    const fetchMyBooks = async () => {
      ...
      try {
        ...
        const data = await getallBooks(1, 100, "", "all", user._id);
        ...
        <div className="md:w-1/3 lg:w-1/4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 text-center sticky top-24">
            ...
            <div className="mt-6">
              <a
                href="/add-book"
                className="block w-full py-2 bg-primary text-white rounded-xl font-bold hover:bg-primary/90 transition-colors"
              >
                إضافة كتاب
              </a>
            </div>
            ...
            {myBooksLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {myBooks.map((book) => (
                  <div
                    key={book._id}
                    className="flex gap-4 p-4 border rounded-xl dark:border-slate-600"
                  >
                    <div className="w-20 h-28 bg-slate-200 rounded overflow-hidden shrink-0">
                      {book.coverImage && (
                        <img
                          src={book.coverImage}
              ...
              <div className="text-center py-10">
                <p className="text-slate-500">لم تقم بإضافة أي كتب بعد.</p>
              </div>
            )}
          </div>
